...
- [ ] **Pydantic 스키마** (`app/schemas/push.py`, `app/schemas/dlq.py`)
...
- [ ] **Push API** (`app/api/push.py`)
  - `POST /push`: idempotency_key 없으면 UUID 발급 → push_logs INSERT(PENDING) → Celery enqueue (랜덤 성공/실패) → 202
  - `GET /push/{push_id}`: push_logs 조회 → 200

- [ ] **DLQ API** (`app/api/dlq.py`)
  - `GET /dlq?limit=20&offset=0`: push_logs WHERE status='DEAD' 조회
  - `POST /dlq/{idempotency_key}/retry`: status=PENDING 업데이트 → Celery re-enqueue → 202
...
- [ ] **단위 테스트 — 멱등성** (`tests/unit/test_idempotency.py`) **[필수]**
  - 신규 키 → True 반환
  - 중복 키 → False 반환

- [ ] **단위 테스트 — Celery 태스크** (`tests/unit/test_push_task.py`) **[필수]**
  - 멱등성 중복 시 태스크 조기 종료 검증
  - 재시도 지수 백오프 (1차 60s) 검증
  - max_retries 초과 시 status=DEAD 검증

- [ ] **단위 테스트 — Fake FCM** (`tests/unit/test_fake_fcm.py`) **[필수]**
